...
import { ZoneViewSkeleton } from '@/components/skeleton/zone_view_skeleton'
import { ArrowIcon, CurrencyIcon } from '@/components/icons'
import { AvatarCircle } from '@/components/ui/avatar_circle'
import { Button } from '@/components/ui/button'
import { LeaderboardView } from '@/components/views/leaderboard_view'
import {
  CONFETTI_CONFIG,
  LEADERBOARD_MOCK,
  MAX_INVITES_LIMIT,
...
import { LeaderboardTops } from '@/components/ui/leaderboard_tops'
      ...
      <div className="py-8 px-4 flex flex-col items-center w-full gap-6">
        ...
        {!!userInitData?.referrals?.referred_users?.length ? (
          <div className="flex flex-col w-full my-4 gap-8 overflow-scroll">
            {userInitData?.referrals?.referred_users.map((u) => (
              <div key={u.tg_id} className="flex gap-4 items-center">
                <AvatarCircle
                ...
                <p className="flex-1 justify-self-end text-right">+1,000 XP</p>
              </div>
            ))}
          </div>
        ) : (
